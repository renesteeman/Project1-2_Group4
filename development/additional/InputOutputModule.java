...
public class InputOutputModule {
    ...
    public InputOutputModule(String path) {
        try {
            ...
            flag = new Vector2d(s1, s2);

            curarray = inp.nextLine().split(" ");
            s1 = curarray[2].replace("(", "").replace(",", "");
            s2 = curarray[3].replace(")", "");
            start = new Vector2d(s1, s2);

            curarray = inp.nextLine().split(" ");
            friction = Double.parseDouble(curarray[2]);

            curarray = inp.nextLine().split(" ");
            maxVelocity = Double.parseDouble(curarray[2]);

            curarray = inp.nextLine().split(" ");
            holeRadius = Double.parseDouble(curarray[2]);
