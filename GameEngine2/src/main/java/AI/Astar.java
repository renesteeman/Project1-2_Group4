...
public class Astar {
	...
	public Astar(PuttingSimulator simulator, double stepDegree, double stepVelocityLength, int numberOfVelocitySteps) {
		...
		oneDimError = 100;//(int)simulator.course.getHoleRadius();
		...
		for (int i = 0; i < used.length; i++) {
			for (int j = 0; j < used[i].length; j++) {
				g[i][j] = 100000;
				h[i][j] = Math.abs(i - goalNode.x) + Math.abs(j - goalNode.y);
				f[i][j] = g[i][j] + h[i][j];
			}
		}
		...
		grid = new VisualizeGrid(used.length, used[0].length, oneDimError);
	...
	public void astar(Vector2d start, Vector2d absStart) {
		...
		f[(int)start.x][(int)start.y] = g[(int)start.x][(int)start.y] + h[(int)start.x][(int)start.y];
		...
		while (queue.size() > 0 && breakRule == 0) {
			...
			for (int angle = -180; angle < 180 && !solved; angle += stepDegree) {
				...
				for (int j = 0; j < numberOfVelocitySteps && !solved; j++) {
					...
					if ((Math.abs(nxtPos.x - node.x) + Math.abs(nxtPos.y - node.y)) > 1)
						continue;
					...
					if (!used[(int)nxtPos.x][(int)nxtPos.y] || tentativeScore < g[(int)nxtPos.x][(int)nxtPos.y]) {
						...
						System.out.println("pushed" + nxtPos);
